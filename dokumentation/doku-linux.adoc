...
- IPv6 -> fe80:0000:0000:0000:2e33:58ff:fefa:27a1 -> 128 Bit
...
Subnetzmaske/CIDR heißt hier _Präfix_

  2001:0db8:85a3:0000:0000:0000:0000:0001/64 ->  /64 ist der Präfix

Es gibt keinen _Broadcast_ mehr. Die Entsprechung hierzu ist der _Multicast_. Hier werden Pakete nicht an *alle* erreichbaren Hosts gesendet, sondern an eine bestimmte *Gruppe* von Hosts, z.B. an alle Router, an alle Drucker oder so.
...
Bei IPv4 wird der Port hinter die IP Adresse durch einen Doppelpunkt getrennt angegeben:

  192.168.2.199:443

Das ist bei IPv6 nicht eindeutig: Ist unten die `443` der Port oder Teil der IPv6 Adresse (`0443`)?

  2001::db8:0000:0000:a00:443

Daher wird die IP Adresse in eckige Klammern gesetzt und der Port wie gewohnt mit einem Doppelpunkt angehängt:
...
- `::/0`: Standardroute
- `::/128`: entspricht `0.0.0.0` unter IPv4, darf keinem Host zugewiesen werden
- `::1/128`: Loopback Adresse
- `fe80::/64`: Link-Local-Unicast Adressen
- `fc00::/7`: "private IP Adressen" _Uniqui Local Addresses_ (ULA)
- `ff00::/8`: Multicast Adressen
- usw. (für uns nicht weiter wichtig)

==== UDP, TCP, ICMP

- `UDP`: _User Datagram Protocal_ - verbindungsloses Protokoll, es findet keine Kontrolle statt, ob Pakete angekommen sind -> schneller, unsicherer (Streaming, Video-Chat, ...)
- `TCP`: _Transmission Control Protocol_ - verbindungsbasiertes Protokoll, es wird geprüft ob alle Pakete ankommen und ggf. nachgeschickt -> langsamer, sicherer (Surfen im Internet, Dateiübertragungen, E-Mail, ...)
- `ICMP`: _Internet Control Message Protocol_ - Kommanodo `ping`

=== wichtige Ports 

In der Datei `/etc/services` werden die Ports für die jeweiligen Services gelistet. Bzw. können Dienste hier nachschauen, welcher Standardport für sie gesetzt ist. Nähere Infos zu der Datei und Ports im Allgemeinen unter `man 5 services`.

Es folgt eine Liste der wichtigsten Ports und zugehöriger Dienste:
